...
const MiniStaff = ({ steps, timeSig }) => {
    ...
    const notes = steps.map(step => {
        ...
        return { 
            ...
            isRest: step.strum === ' ',
            // Only beam if note is short AND NOT A REST
        ...
        <svg width={width} height={height} className="opacity-70">
            ...
            {groups.map((group, gIdx) => {
                ...
                if (isBeam) {
                    ...
                    if (note.isRest) {
                        // Simplified Rest Dot for mini preview
                        return <circle key={gIdx} cx={x} cy={y-5} r="1.5" fill="#64748b" />;
                    }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-900/50">
        <div className="p-2 border-b border-white/5 flex gap-2 shrink-0">
            ...
            <button onClick={handleGenerate} className="flex-1 bg-orange-500/10 hover:bg-orange-500 text-orange-500 hover:text-white text-xs font-bold py-1 px-2 rounded transition-colors flex items-center justify-center gap-1 border border-orange-500/20">
